...
*To prove.* An element $x \in A_n$ can be written as a finite product of terms in $\Sigma$.
...
To find a representation of an arbitrary $x \in A_n$ we proceed by strong induction on the number of elements that $x$ moves. For the base case, observe that if $x \in A_n$ moves fewer than three elements, then $x = \mathrm{id}$, and we're done. Now take $x$ to move finitely many elements. Now, there's a $\sigma_r \in \Sigma$ such that $\sigma_r^{-1} x$ can be represented as a finite product of $3$-cycles in $\Sigma$, e.g., $$\sigma_r^{-1} x = \sigma_1 \sigma_2 \cdots \sigma_{r-1}, \quad\text{ thus }\quad x =\sigma_1 \cdots \sigma_{r}.$$

Which is what we wanted. \qedsymbol
